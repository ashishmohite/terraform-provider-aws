...
* `restore_to_point_in_time` - (Optional, Forces new resource) A configuration block for restoring a DB instance to an arbitrary point in time. Requires the `identifier` argument to be set. See [Restore To Point In Time](#restore-to-point-in-time) below for details.
...
-> **Note:** You can restore to a point up to the number of days specified in the source DB's `backup_retention_period`.
...
* `source_db_instance_identifier` - (Required) The identifier of the source DB instance from which to restore. Must match the identifier of an existing DB instance.
